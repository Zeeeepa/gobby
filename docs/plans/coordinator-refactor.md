# Coordinator Refactor: Options

## Current State

### What the coordinator does

The coordinator (`coordinator.yaml`) is a pipeline-type workflow that processes a task queue by spawning developer, QA, and merge agents in sequence. Its 14 steps do five distinct things:
...
**1. Self-recursive pipeline.** The coordinator calls `invoke_pipeline: coordinator` to loop, passing `_current_iteration` as a parameter. Pipelines are designed for linear execution. This simulates a loop in a system that doesn't have one.
...
- Coordinator calls `close_task` (coordinator.yaml, close_task step)

No single owner. If any agent fails mid-lifecycle, the task is left in an ambiguous state.

**3. Sequential-only execution.** The coordinator spawns one developer, waits, spawns one QA, waits, spawns one merge, waits. There's no parallelism — it can only process one task at a time per loop iteration. The orchestration tools were specifically built for parallel dispatch.

**4. Clone lifecycle coupled to task discovery.** The clone is created after `find_work` but before `spawn_developer`, tying infrastructure setup to task resolution. The clone API shouldn't need to know about task state, and the coordinator shouldn't need to manage clone IDs.
...
| Tool | What it does | Coordinator equivalent |
|------|-------------|----------------------|
