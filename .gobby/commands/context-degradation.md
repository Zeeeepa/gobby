...
The RULER benchmark delivers sobering findings: only 50% of models claiming 32K+ context maintain satisfactory performance at 32K tokens. GPT-4.1 shows the least degradation among current models, while many still drop 30+ points at extended contexts. Near-perfect scores on simple needle-in-haystack tests do not translate to real long-context understanding.

**Model-Specific Degradation Thresholds**
| Model | Degradation Onset | Severe Degradation | Notes |
|-------|-------------------|-------------------|-------|
| GPT-4.1 | ~500K tokens | ~800K tokens | ~1M context window, best overall degradation resistance |
| Claude Opus 4.1 | ~100K tokens | ~180K tokens | Default ~200K, enterprise/beta up to ~500K–1M |
| Claude Sonnet 4 | ~80K tokens | ~150K tokens | Default ~200K, optimized for agents and coding tasks |
| Gemini Pro | ~800K tokens | ~1.5M tokens | ~1M–2M context window, native multimodality |

**Model-Specific Behavior Patterns**
Different models exhibit distinct failure modes under context pressure:

- **Claude 4 series**: Lowest hallucination rates with calibrated uncertainty. Claude Opus 4.1 achieves strong results on SWE-bench Verified. Tends to refuse or ask clarification rather than fabricate.
- **GPT-4.1**: Large context window (~1M tokens) with strong degradation resistance. Multiple modes available for different latency/quality tradeoffs.
- **Gemini Pro**: Native multimodality with 1M–2M context window. Strong at multi-modal reasoning across text, code, images, audio, and video.

These patterns inform model selection for different use cases. High-stakes tasks benefit from Claude 4's conservative approach; speed-critical tasks may use faster model variants.
