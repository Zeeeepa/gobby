---
description: This skill should be used when the user asks to "/gobby-memory", "remember", "recall", "forget memory". Manage persistent memories across sessions - store, search, delete, and list memories.
---
...
Example: `/gobby-memory recall testing best practices` → `recall(query="testing best practices")`
Example: `/gobby-memory recall tag:security` → `recall(tags=["security"])`
...
Example: `/gobby-memory forget mm-abc123` → `forget(memory_id="mm-abc123")`
...
Example: `/gobby-memory list tag:workflow` → `list_memories(tags=["workflow"])`
...
Example: `/gobby-memory stats` → `get_stats()`
