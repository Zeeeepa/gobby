...
class TestSaveArtifact:
    ...
    def test_save_with_required_params(self, artifacts_registry, mock_artifact_manager) -> None:
    ...
    def test_save_with_all_optional_params(self, artifacts_registry, mock_artifact_manager) -> None:
    ...
    def test_save_auto_classifies_when_no_type(self, artifacts_registry, mock_artifact_manager) -> None:
    ...
    def test_save_exception_handling(self, artifacts_registry, mock_artifact_manager) -> None:
...
class TestDeleteArtifact:
    ...
    def test_delete_existing(self, artifacts_registry, mock_artifact_manager) -> None:
        """Test delete_artifact removes existing artifact."""
        mock_artifact_manager.delete_artifact.return_value = True
        result = call_tool(artifacts_registry, "delete_artifact", artifact_id="art-123")
        assert result["success"] is True
        mock_artifact_manager.delete_artifact.assert_called_once_with("art-123")

    def test_delete_nonexistent(self, artifacts_registry, mock_artifact_manager) -> None:
        """Test delete_artifact returns error for non-existent ID."""
        mock_artifact_manager.delete_artifact.return_value = False
        result = call_tool(artifacts_registry, "delete_artifact", artifact_id="nonexistent")
        assert result["success"] is False
...
class TestTagArtifact:
    ...
    def test_tag_artifact(self, artifacts_registry, mock_artifact_manager) -> None:
        """Test tag_artifact adds tag."""
        result = call_tool(
            artifacts_registry, "tag_artifact", artifact_id="art-123", tag="important"
        )
        assert result["success"] is True
        mock_artifact_manager.add_tag.assert_called_once_with("art-123", "important")

    def test_tag_artifact_idempotent(self, artifacts_registry, mock_artifact_manager) -> None:
        """Test tag_artifact is idempotent."""
        mock_artifact_manager.add_tag.return_value = True
        result = call_tool(
            artifacts_registry, "tag_artifact", artifact_id="art-123", tag="duplicate"
        )
        assert result["success"] is True

    def test_untag_artifact(self, artifacts_registry, mock_artifact_manager) -> None:
        """Test untag_artifact removes tag."""
        mock_artifact_manager.remove_tag.return_value = True
        result = call_tool(
            artifacts_registry, "untag_artifact", artifact_id="art-123", tag="old-tag"
        )
        assert result["success"] is True

    def test_untag_nonexistent(self, artifacts_registry, mock_artifact_manager) -> None:
        """Test untag_artifact returns error for non-existent tag."""
        mock_artifact_manager.remove_tag.return_value = False
        result = call_tool(
            artifacts_registry, "untag_artifact", artifact_id="art-123", tag="missing"
        )
        assert result["success"] is False
...
class TestListArtifactsByTask:
    ...
    def test_list_by_task(self, artifacts_registry, mock_artifact_manager) -> None:
        ...
        result = call_tool(
            artifacts_registry, "list_artifacts_by_task", task_id="task-001"
        )
        assert result["success"] is True
        assert result["count"] == 1

    def test_list_by_task_with_type_filter(self, artifacts_registry, mock_artifact_manager) -> None:
    ...
    def test_list_by_task_empty(self, artifacts_registry, mock_artifact_manager) -> None:
        """Test list_artifacts_by_task with no matches."""
        mock_artifact_manager.list_by_task.return_value = []
        result = call_tool(
            artifacts_registry, "list_artifacts_by_task", task_id="no-artifacts"
        )
        assert result["success"] is True
...
class TestRegistryHasWriteTools:
    ...
    def test_registry_has_write_tools(self, artifacts_registry) -> None:
