...
class TestRecallMemory:
    ...
    @pytest.mark.asyncio
    async def test_recall_memory_success(self, memory_registry, mock_memory_manager):
        ...
        with patch("gobby.utils.project_context.get_project_context", return_value={"id": "proj-1"}):
            result = await memory_registry.call(
                "recall_memory", {"query": "test query", "limit": 5}
            )
    ...
    @pytest.mark.asyncio
    async def test_recall_memory_with_filters(self, memory_registry, mock_memory_manager):
    ...
    @pytest.mark.asyncio
    async def test_recall_memory_error(self, memory_registry, mock_memory_manager):
