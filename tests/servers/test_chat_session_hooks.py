...
class TestChatSessionHooks:
    ...
    async def test_build_prompt_hook(self, session: ChatSession) -> None:
        ...
        mock_cb.assert_awaited_once_with({"prompt": "testing auth", "source": "claude_sdk_web_chat"})
        ...
        session._session_manager_ref.update.assert_awaited_once_with("db-id", transcript_path="/var/tmp/transcript.gz")
