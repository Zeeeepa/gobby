...
class TestGetMessagesRendered:
    ...
    def test_get_messages_legacy_format(
        ...
        """Test that format=legacy uses message_manager."""
        session = session_storage.register(
        ...
        mock_msg_manager = AsyncMock()
        mock_msg_manager.get_messages = AsyncMock(return_value=[{"role": "user", "content": "hi"}])
        mock_msg_manager.count_messages = AsyncMock(return_value=1)
        server.message_manager = mock_msg_manager
        ...
        mock_msg_manager.get_messages.assert_called_once_with(
