...
class TestPackCommand:
    ...
    def test_pack_no_gobby_home(self, mock_home, runner: CliRunner):
    ...
    def test_pack_dry_run(self, mock_docker, mock_daemon, mock_home, tmp_path, runner: CliRunner):
    ...
    @patch("gobby.cli.pack._daemon_is_running", return_value=False)
    @patch("gobby.cli.pack._docker_available", return_value=False)
    def test_pack_success(self, mock_docker, mock_daemon, mock_home, tmp_path, runner: CliRunner):
        fake_home = tmp_path / ".gobby"
        fake_home.mkdir()
    ...
    @patch("gobby.cli.pack._docker_available", return_value=False)
    def test_pack_daemon_lifecycle(self, mock_docker, mock_start, mock_stop, mock_daemon, mock_home, tmp_path, runner: CliRunner):
        fake_home = tmp_path / ".gobby"
        fake_home.mkdir()
...
class TestUnpackCommand:
    ...
    @patch("gobby.cli.pack.GOBBY_HOME")
    def test_unpack_dry_run(self, mock_home, tmp_path, runner: CliRunner):
    ...
    def test_unpack_success(self, mock_hooks, mock_docker, mock_daemon, mock_home, tmp_path, runner: CliRunner):
        archive = self._create_fake_archive(tmp_path)
    ...
    @patch("gobby.cli.pack.GOBBY_HOME")
    def test_unpack_aborts_if_exists(self, mock_home, tmp_path, runner: CliRunner):
        archive = self._create_fake_archive(tmp_path)
    ...
    @patch("gobby.cli.pack._docker_available", return_value=False)
    @patch("gobby.cli.pack.install_git_hooks", return_value={"success": True, "installed": []})
    def test_unpack_force(self, mock_hooks, mock_docker, mock_daemon, mock_home, tmp_path, runner: CliRunner):
        archive = self._create_fake_archive(tmp_path)
