...
class TestCheckGeminiTipPrompts:
    """Tests for Gemini CLI tip prompt auto-dismissal."""

    @pytest.mark.asyncio
    async def test_dismisses_tip_prompt(
        self,
        monitor: AgentLifecycleMonitor,
        agent_run_manager: LocalAgentRunManager,
        sample_session: dict,
    ) -> None:
        """Tip prompt with (esc to cancel) is dismissed via Escape."""
        run = _make_terminal_run(
            agent_run_manager,
            sample_session,
            run_id="run-tip",
            tmux_session_name="gobby-tip-test",
        )
        ...
        monitor._tmux = AsyncMock()
        monitor._tmux.capture_pane = AsyncMock(return_value=pane_output)
    ...
    async def test_no_dismiss_on_normal_output(
        ...
        """Normal pane output does not trigger tip dismissal."""
        _make_terminal_run(
            agent_run_manager,
            sample_session,
            run_id="run-normal",
            tmux_session_name="gobby-normal-test",
        )

        monitor._tmux = AsyncMock()
        monitor._tmux.capture_pane = AsyncMock(return_value="Running tests...\n$ pytest -v\n")
        monitor._tmux.send_keys = AsyncMock(return_value=True)

        handled = await monitor.check_gemini_tip_prompts()

        assert handled == 0
        monitor._tmux.send_keys.assert_not_called()

    @pytest.mark.asyncio
    async def test_handles_multiple_agents(
        self,
        monitor: AgentLifecycleMonitor,
        agent_run_manager: LocalAgentRunManager,
        sample_session: dict,
    ) -> None:
        """Multiple agents with tip prompts are all dismissed."""
        _make_terminal_run(
            agent_run_manager,
            sample_session,
            run_id="run-tip-1",
            tmux_session_name="gobby-tip-1",
        )
        _make_terminal_run(
            agent_run_manager,
            sample_session,
            run_id="run-tip-2",
            tmux_session_name="gobby-tip-2",
        )

        tip_output = " ⠏ Restrict built-in tools (settings.json)… (esc to cancel, 30s)\n"

        monitor._tmux = AsyncMock()
        monitor._tmux.capture_pane = AsyncMock(return_value=tip_output)
        monitor._tmux.send_keys = AsyncMock(return_value=True)

        handled = await monitor.check_gemini_tip_prompts()

        assert handled == 2
        assert monitor._tmux.send_keys.call_count == 2
