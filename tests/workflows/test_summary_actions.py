...
# =============================================================================
# Tests for generate_summary with compressor
# =============================================================================


class TestGenerateSummaryWithCompressor:
    """Tests for generate_summary() compressor integration."""

    @pytest.fixture
    def mock_compressor(self):
        """Create a mock compressor that returns compressed content."""
        compressor = MagicMock()
        compressor.compress.return_value = "Compressed transcript content"
        return compressor

    @pytest.mark.asyncio
    async def test_compressor_none_uses_default_max_turns(
        self,
        mock_session_manager,
        mock_llm_service,
        mock_transcript_processor,
        ...
        """Test that compressor=None uses default max_turns of 50."""
        transcript_file = tmp_path / "transcript.jsonl"
        with open(transcript_file, "w") as f:
            f.write(json.dumps({"message": {"role": "user", "content": "Test"}}) + "\n")
        ...
        # Verify extract_turns_since_clear was called with default max_turns=50
        mock_transcript_processor.extract_turns_since_clear.assert_called_once()
        call_kwargs = mock_transcript_processor.extract_turns_since_clear.call_args
        assert call_kwargs.kwargs.get("max_turns") == 50

    @pytest.mark.asyncio
    async def test_compressor_provided_increases_max_turns(
        self,
        mock_session_manager,
        mock_llm_service,
        mock_transcript_processor,
        ...
        """Test that providing compressor increases max_turns to 100."""
        transcript_file = tmp_path / "transcript.jsonl"
        with open(transcript_file, "w") as f:
            f.write(json.dumps({"message": {"role": "user", "content": "Test"}}) + "\n")
        ...
        # Verify extract_turns_since_clear was called with max_turns=100
        mock_transcript_processor.extract_turns_since_clear.assert_called_once()
        call_kwargs = mock_transcript_processor.extract_turns_since_clear.call_args
        assert call_kwargs.kwargs.get("max_turns") == 100

    @pytest.mark.asyncio
    async def test_compressor_called_with_transcript_and_handoff_context(
        self,
        mock_session_manager,
        mock_llm_service,
        mock_transcript_processor,
        ...
        """Test that compressor.compress() is called with correct arguments."""
        transcript_file = tmp_path / "transcript.jsonl"
        with open(transcript_file, "w") as f:
            f.write(json.dumps({"message": {"role": "user", "content": "Hello world"}}) + "\n")

        session = MagicMock()
        session.jsonl_path = str(transcript_file)
        mock_session_manager.get.return_value = session

        test_turns = [{"message": {"role": "user", "content": "Hello world"}}]
        mock_transcript_processor.extract_turns_since_clear.return_value = test_turns
        mock_transcript_processor.extract_last_messages.return_value = []
        ...
        # Verify compressor.compress was called
        mock_compressor.compress.assert_called_once()
        call_args = mock_compressor.compress.call_args

        # Check that transcript content was passed (contains formatted turn)
        assert "[Turn 1 - user]: Hello world" in call_args[0][0]
        # Check context_type is "handoff"
        assert call_args.kwargs.get("context_type") == "handoff"

    @pytest.mark.asyncio
    async def test_compressed_transcript_passed_to_llm(
        self,
        mock_session_manager,
        mock_llm_service,
        mock_transcript_processor,
        ...
        """Test that compressed transcript is passed to LLM context."""
        transcript_file = tmp_path / "transcript.jsonl"
        with open(transcript_file, "w") as f:
            f.write(json.dumps({"message": {"role": "user", "content": "Test content"}}) + "\n")
        ...
        # Set up compressor to return specific compressed content
        mock_compressor.compress.return_value = "COMPRESSED: Test content summary"
        ...
        # Verify LLM received compressed transcript
        provider = mock_llm_service.get_default_provider()
        call_kwargs = provider.generate_summary.call_args.kwargs
        assert call_kwargs["context"]["transcript_summary"] == "COMPRESSED: Test content summary"

    @pytest.mark.asyncio
    async def test_no_compressor_passes_uncompressed_transcript_to_llm(
        self,
        mock_session_manager,
        mock_llm_service,
        mock_transcript_processor,
        ...
        """Test that without compressor, uncompressed transcript goes to LLM."""
        transcript_file = tmp_path / "transcript.jsonl"
        with open(transcript_file, "w") as f:
            f.write(json.dumps({"message": {"role": "user", "content": "Original text"}}) + "\n")
        ...
        mock_transcript_processor.extract_turns_since_clear.return_value = [
            {"message": {"role": "user", "content": "Original text"}}
        ]
        mock_transcript_processor.extract_last_messages.return_value = []
        ...
        # Verify LLM received original formatted transcript
        provider = mock_llm_service.get_default_provider()
        call_kwargs = provider.generate_summary.call_args.kwargs
        assert "[Turn 1 - user]: Original text" in call_kwargs["context"]["transcript_summary"]

    @pytest.mark.asyncio
    async def test_generate_summary_with_compressor_success(
        self,
        mock_session_manager,
        mock_llm_service,
        mock_transcript_processor,
        ...
        """Test complete generate_summary with compressor returns success."""
        transcript_file = tmp_path / "transcript.jsonl"
        with open(transcript_file, "w") as f:
            f.write(json.dumps({"message": {"role": "user", "content": "Help"}}) + "\n")
...
# =============================================================================
# Tests for generate_handoff with compressor
# =============================================================================


class TestGenerateHandoffWithCompressor:
    """Tests for generate_handoff() compressor integration."""

    @pytest.fixture
    def mock_compressor(self):
        """Create a mock compressor."""
        compressor = MagicMock()
        compressor.compress.return_value = "Compressed handoff content"
        return compressor

    @pytest.mark.asyncio
    async def test_handoff_passes_compressor_to_generate_summary(
        self,
        mock_session_manager,
        mock_llm_service,
        mock_transcript_processor,
        ...
        """Test that generate_handoff passes compressor to generate_summary."""
        transcript_file = tmp_path / "transcript.jsonl"
        with open(transcript_file, "w") as f:
            f.write(json.dumps({"message": {"role": "user", "content": "Test"}}) + "\n")
        ...
        # Verify compressor was used
        mock_compressor.compress.assert_called_once()

    @pytest.mark.asyncio
    async def test_handoff_with_compressor_and_mode(
        self,
        mock_session_manager,
        mock_llm_service,
        mock_transcript_processor,
        ...
        """Test handoff with both compressor and compact mode."""
        transcript_file = tmp_path / "transcript.jsonl"
        with open(transcript_file, "w") as f:
            f.write(json.dumps({"message": {"role": "user", "content": "Test"}}) + "\n")
