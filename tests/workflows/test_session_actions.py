...
@pytest.mark.asyncio
async def test_handle_start_new_session() -> None:
    from gobby.workflows.session_actions import handle_start_new_session
    ...
    with patch("gobby.workflows.session_actions.subprocess.Popen") as mock_popen:
        mock_popen.return_value.pid = 1
        result = await handle_start_new_session(ctx, command="claude")
    assert result is not None


@pytest.mark.asyncio
async def test_handle_mark_session_status() -> None:
    from gobby.workflows.session_actions import handle_mark_session_status

    ctx = MagicMock()
    ctx.session_manager = MagicMock()
    ctx.session_id = "s1"

    result = await handle_mark_session_status(ctx, status="active")
    assert result is not None
    assert result["status_updated"] is True


@pytest.mark.asyncio
async def test_handle_switch_mode() -> None:
    from gobby.workflows.session_actions import handle_switch_mode

    ctx = MagicMock()
    result = await handle_switch_mode(ctx, mode="PLAN")
    assert result is not None
    assert result["mode_switch"] == "PLAN"
