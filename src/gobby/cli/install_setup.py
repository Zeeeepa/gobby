...
# ── Local embeddings (llama-cpp-python + GGUF model) ──────────────────

_GGUF_MODEL_URL = "https://huggingface.co/nomic-ai/nomic-embed-text-v1.5-GGUF/resolve/main/nomic-embed-text-v1.5.Q8_0.gguf"
_GGUF_MODEL_FILENAME = "nomic-embed-text-v1.5.Q8_0.gguf"
_GGUF_MODEL_DIR = Path("~/.gobby/models").expanduser()


def _install_llama_cpp_python() -> bool:
    """Install llama-cpp-python via uv sync --extra local-embeddings.
    ...
    try:
        ...
        if project_root is None:
            logger.warning("local-embeddings: could not find pyproject.toml")
            return False
        ...
        logger.warning("local-embeddings: uv sync failed: %s", e)
        return False


def _install_cmake() -> bool:
    """Install cmake as a build dependency for llama-cpp-python.

    Tries brew (macOS) or apt-get (Linux).
    ...
    if sys.platform == "darwin":
        if shutil.which("brew"):
            try:
                click.echo("  Installing cmake via Homebrew...")
                result = subprocess.run(
                    ["brew", "install", "cmake"],
                    capture_output=True,
                    text=True,
        ...
        if shutil.which("apt-get"):
            try:
                click.echo("  Installing cmake via apt-get...")
                result = subprocess.run(
...
def _download_gguf_model() -> bool:
    """Download the nomic-embed-text-v1.5 Q8_0 GGUF model (~139MB).

    Uses urllib to match the existing install_setup.py download pattern.
    Skips if model already exists.

    Returns:
        ``True`` on success or already present, ``False`` on failure.
    """
    model_path = _GGUF_MODEL_DIR / _GGUF_MODEL_FILENAME

    if model_path.exists():
        size_mb = model_path.stat().st_size / (1024 * 1024)
        logger.info("GGUF model already downloaded: %s (%.1f MB)", model_path, size_mb)
        return True

    _GGUF_MODEL_DIR.mkdir(parents=True, exist_ok=True)
    tmp_path = model_path.with_suffix(".tmp")

    try:
        click.echo("  Downloading embedding model (~139 MB)...")
        req = Request(_GGUF_MODEL_URL, headers={"User-Agent": "gobby-installer/1.0"})
        # HuggingFace uses HTTPS — validated by _urlopen_https
        with _urlopen_https(req, timeout=300) as resp:
        ...
        size_mb = model_path.stat().st_size / (1024 * 1024)
        click.echo(f"  Downloaded {_GGUF_MODEL_FILENAME} ({size_mb:.1f} MB)")
        return True

    except (URLError, OSError) as e:
        logger.warning("GGUF model download failed: %s", e)
        if tmp_path.exists():
            tmp_path.unlink()
        return False


def _install_local_embeddings(force: bool = False) -> dict[str, Any]:
    """Install local embeddings: llama-cpp-python package + GGUF model.

    Installation priority:
      1. ``uv sync --extra local-embeddings`` (uses wheel if available)
      2. Install cmake, then retry ``uv sync`` (build from source)

    After package install, downloads the nomic-embed-text-v1.5 Q8_0 GGUF
    model (~139MB) to ``~/.gobby/models/``.

    Args:
        force: Re-install even if already present.

    Returns:
        Dict with keys: ``installed``, ``skipped``, ``method``, ``reason``.
    """
    ...
    # Fallback chain for package install
    method = None

    if _install_llama_cpp_python():
        method = "uv-wheel"
    else:
        # Wheel failed — try installing cmake then retry
        click.echo("  Pre-built wheel unavailable, trying build from source...")
        if _install_cmake() and _install_llama_cpp_python():
            method = "uv-source"
        else:
            return {
                "installed": False,
                "skipped": False,
                "reason": "llama-cpp-python installation failed (no wheel, cmake build failed)",
            }
    ...
    if not _download_gguf_model():
        return {
            ...
            "skipped": False,
            "method": method,
            "reason": "package installed but model download failed — will auto-download on first use",
        }
