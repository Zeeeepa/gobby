...
description: Internal guidance for composing Codex and GPT-5.4 prompts for coding, review, diagnosis, and research tasks
...
Use this skill when composing prompts for `codex exec` or `codex review` tasks.
...
## How to choose invocation

- Use `codex review` when the job is reviewing local git changes. It carries its own review contract.
- Use `codex exec -s read-only` for diagnosis, planning, or research without edits.
- Use `codex exec -s workspace-write` for implementation and fix tasks.
- Use `codex exec resume --last` for follow-up instructions on the same Codex thread. Send only the delta instruction instead of restating the whole prompt unless the direction changed materially.
