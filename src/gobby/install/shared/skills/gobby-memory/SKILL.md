---
name: gobby-memory
description: This skill should be used when the user asks to "/gobby-memory", "create", "recall", "delete memory". Manage persistent memories across sessions - store, search, delete, update, and list memories.
---
...
### `/gobby-memory create <content>` - Store a memory
...
Example: `/gobby-memory create Always use pytest fixtures for test setup`
→ `create_memory(content="Always use pytest fixtures for test setup", tags="testing")`

Example: `/gobby-memory create [critical] Never commit .env files`
...
### `/gobby-memory delete <memory-id>` - Delete a memory
Call `gobby-memory.delete_memory` with:
- `memory_id`: (required) The memory ID to delete

Example: `/gobby-memory delete mm-abc123` → `delete_memory(memory_id="mm-abc123")`
...
### `/gobby-memory graph` - Export memory graph
...
Example: `/gobby-memory graph` → `export_memory_graph()`

## Response Format

After executing the appropriate MCP tool, present the results clearly:
- For create: Confirm storage with memory ID
...
- For graph: Confirm export with file path
...
- create, recall, delete, list, show, update, related, stats, graph
